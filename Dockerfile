...
RUN /bin/bash -c 'sudo apt-get update && sudo apt-get install -y libarmadillo-dev ros-melodic-nlopt libdw-dev git && \
	cd /home/docker && \
	mkdir -p FUEL/catkin_fuel/src && source /opt/ros/melodic/setup.bash && cd FUEL/catkin_fuel && catkin_make && \
	source devel/setup.bash && cd src && git clone https://github.com/t-thanh/FUEL && \
	cd .. && catkin_make'
RUN echo "source /home/docker/FUEL/catkin_fuel/devel/setup.bash" > ~/.bashrc
# Launch terminator
CMD ["terminator"]
